...
def _build_output_paths(
    ...
    *,
    ...
    if unique_output:
        base_source = f"{image_path.stem}_{source_prefix}_{stamp}"
        base_target = f"{image_path.stem}_{target_prefix}_{stamp}"
    else:
        base_source = source_prefix
        base_target = target_prefix
    source_mask = output_dir / f"{base_source}.npy"
    source_meta = output_dir / f"{base_source}.json"
    target_mask = output_dir / f"{base_target}.npy"
    target_meta = output_dir / f"{base_target}.json"
    return source_mask, source_meta, target_mask, target_meta, stamp
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Interactively collect two region masks (source/person and target) from bounding boxes."
    )
    parser.add_argument("--image-path", default=DEFAULT_IMAGE_PATH, help="Path to the source image.")
    parser.add_argument(
        "--output-dir",
        default="region_masks",
        help="Directory to store the generated masks and metadata.",
    )
    parser.add_argument(
        "--input-mode",
    ...
    parser.add_argument(
        "--source-prefix",
        default="source_region_mask",
        help="Base filename for the source/person mask when --unique-output is not used.",
    )
    parser.add_argument(
        "--target-prefix",
        default="target_region_mask",
        help="Base filename for the target mask when --unique-output is not used.",
    )
    parser.add_argument(
        "--unique-output",
        action="store_true",
        help="Append image stem and timestamp to output names for both masks.",
    )
    parser.add_argument(
        "--no-gui",
        action="store_true",
    ...
    source_bounds, source_norm, source_mode = _collect_region(
        image=image,
    ...
    target_bounds, target_norm, target_mode = _collect_region(
        image=image,
        image_size=image_size,
        input_mode=args.input_mode,
        label="TARGET",
    ...
    source_mask = _create_mask(image_size, source_bounds)
    target_mask = _create_mask(image_size, target_bounds)

    source_mask_path, source_meta_path, target_mask_path, target_meta_path, stamp = _build_output_paths(
        image_path,
        output_dir,
        args.source_prefix,
        args.target_prefix,
    ...
    np.save(source_mask_path, source_mask.astype(np.uint8))
    ...
    source_metadata = {
        **base_metadata,
        ...
        "detected_mode": source_mode,
        "pixel_bounds": {
            "x_min": source_bounds[0],
            "y_min": source_bounds[1],
            "x_max": source_bounds[2],
            "y_max": source_bounds[3],
        },
        "normalized_bounds": {
            "x_min": source_norm[0],
            "y_min": source_norm[1],
    ...
    target_metadata = {
        **base_metadata,
        ...
        "detected_mode": target_mode,
        "pixel_bounds": {
            "x_min": target_bounds[0],
            "y_min": target_bounds[1],
            "x_max": target_bounds[2],
            "y_max": target_bounds[3],
        },
        "normalized_bounds": {
            "x_min": target_norm[0],
            "y_min": target_norm[1],
    ...
    print("\n✅ Dual region masks created successfully!")
    print(f"Source mask saved to: {source_mask_path}")
    print(f"Target mask saved to: {target_mask_path}")
    print("Metadata:")
    print(f" - Source: {source_meta_path}")
    print(f" - Target: {target_meta_path}")
    print(
        "\nUse these paths with run_dual_region_extraction.py via "
        "--source-mask-path and --target-mask-path."
    )
